...
mod tests {
    ...
    use crate::gtp_v2::packet::messages::information_elements::{InformationElementType, recovery};

    #[test]
    fn test_generate() {
        let mut buffer = [0; MTU];

        let m = Message::new(recovery::InformationElement::new(0xF, 0).unwrap());
    ...
    #[test]
    fn test_length() {
        let m = Message::new(recovery::InformationElement::new(0xAB, 0).unwrap());
        assert_eq!(m.length(), 5); // Length of the recovery IE
    }

    #[test]
    fn test_message_type() {
        let m = Message::new(recovery::InformationElement::new(0xCD, 0).unwrap());
